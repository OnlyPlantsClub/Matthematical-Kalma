...
This is in-process authenticity only: it proves issuance by the canonical resolver in the current trusted runtime. The opaque authority cannot survive serialization or process boundaries. Durable authenticated evidence requires a trusted persistence boundary, signed record, keyed integrity mechanism or equivalent governance decision.
...
`validated-result-graph-v1` requires a caller-declared `snapshotRef`, `authoritative_snapshot` completeness, one canonical result scope, unique observations, complete parents, no cycles or forks, exact correction authority and valid ordering. Graph inventory and observation references are ordered by `observationRef` with an explicit UTF-16 code-unit comparator; roots, successor-map keys and each successor list use the same comparator. Selected lineage is separately current-to-root. No retained graph collection depends on caller array order, locale comparison or sort stability. A fork fails before either branch can settle; multiple unlinked roots quarantine the graph. The selected observation must be a current leaf whose lineage covers the snapshot.
