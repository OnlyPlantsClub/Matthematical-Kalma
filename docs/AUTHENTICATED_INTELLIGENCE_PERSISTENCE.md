...
`rotateKey()` is the bounded public repository operation for rotation. A single D1 batch conditionally appends the expected active key's verify-only event, registers the exact immutable replacement identity, appends its active event, writes bounded audit evidence, and inserts an immutable rotation receipt. The receipt trigger proves the prior link, exact replacement identity, original activation, current old/new statuses and audit row. A zero-row/stale demotion, reused contradictory identity or partial write therefore fails inside SQL and rolls back. The rotation ID is the idempotency identity: an exact authoritative reread returns `idempotent`; any field mismatch returns `rotation_collision`. Competing rotations from one expected event can produce at most one receipt.
