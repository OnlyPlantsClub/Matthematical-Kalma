...
TASK-17 now governs retention through `intelligence-retention/1`: zero default retention for successfully validated raw provider payloads, a mandatory rights schedule/disposition for every provider-derived observation, provisional maximums that never override a shorter provider/legal/purpose period, and no automatic post-termination survival for hashes or derived facts. `mk-intelligence-record-jcs/1` governs future authenticated persistence and revalidation. Neither decision changes this in-memory contract.

Deferred: paid-provider selection, automated acquisition, provider-specific legal/licensing confirmation, provider timestamp trust and skew allowances, sport/market/operator freshness selection, persistent correction storage, D1 migrations, results, APIs, UI, forecasting, recommendations, staking and wager execution.
