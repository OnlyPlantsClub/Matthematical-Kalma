...
test('migration is forward-only, scoped and makes runtime deletion impossible',()=>{
 ...
 for(const table of ['intelligence_rights_schedules','authenticated_intelligence_records','intelligence_record_parents','intelligence_record_commits','intelligence_stream_heads','intelligence_quarantine_records','intelligence_audit_events','intelligence_retention_events','intelligence_keys','intelligence_key_status_events','intelligence_head_transitions','intelligence_finalization_receipts','governed_deletion_authorizations'])assert.match(migration,new RegExp(`CREATE TABLE ${table}`));
 ...
 assert.match(migration,/UNIQUE\(environment_id,idempotency_key\)/);assert.match(migration,/multiple_active_keys/);assert.match(migration,/head_finalization_failed/);assert.doesNotMatch(migration,/WHEN NOT EXISTS\(SELECT 1 FROM governed_deletion_authorizations/);
});
